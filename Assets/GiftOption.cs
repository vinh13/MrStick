...
public class GiftOption : MonoBehaviour
{
	...
	void CreateGem (object ob, bool b)
	{
		if (!b) {
			if (currentGif != null)
				Destroy (currentGif);
			GameObject go = Instantiate (Resources.Load<GameObject> ("CardGifts/Gem"), transform);
	...
	void CreateKey (object ob, bool b)
	{
		if (!b) {
			if (currentGif != null)
				Destroy (currentGif);
			GameObject go = Instantiate (Resources.Load<GameObject> ("CardGifts/Key"), transform);
			go.GetComponent<RollGold> ().SetGold ((string)ob);
			go.gameObject.SetActive (true);
			currentGif = go;
		} else {
	...
	void CreateGold (object ob, bool b)
	{
		if (!b) {
			if (currentGif != null)
				Destroy (currentGif);
			GameObject go = Instantiate (Resources.Load<GameObject> ("CardGifts/Gold"), transform);
			go.GetComponent<RollGold> ().SetGold ((string)ob);
			go.gameObject.SetActive (true);
			currentGif = go;
		} else {
	...
	void CreateSkin (object ob, bool b)
	{
		if (!b) {
			if (currentGif != null)
				Destroy (currentGif);
			GameObject go = Instantiate (Resources.Load<GameObject> ("CardGifts/Skin"), transform);
			go.GetComponent<RollSkin> ().SetSkin ((EquipConfig)ob);
			go.gameObject.SetActive (true);
			currentGif = go;
		} else {
			RollSkin rollSkin = currentGif.GetComponent<RollSkin> ();
			if (rollSkin != null) {
				rollSkin.SetSkin ((EquipConfig)ob);
			}
		}
	}
	...
	void CreateBoot (object ob, bool b)
	{
		...
		if (!b) {
			if (currentGif != null)
				Destroy (currentGif);
			GameObject go = Instantiate (Resources.Load<GameObject> ("CardGifts/Item"), transform);
